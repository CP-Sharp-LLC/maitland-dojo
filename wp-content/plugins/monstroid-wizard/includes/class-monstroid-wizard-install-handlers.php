...
if ( ! class_exists( 'monstroid_wizard_install_handlers' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class monstroid_wizard_install_handler {
		...
		function __construct( $install_type ) {
			...
			$this->steps = apply_filters(
				'monstroid_wizard_installation_steps',
				array(
					'install-framework',
					'install-theme',
					'activate-theme',
			 ...
			 */
			$this->last_step = apply_filters( 'monstroid_wizard_installation_last_step', 'install-plugins' );
			...
			add_action( 'wp_ajax_monstroid_wizard_install_step', array( $this, 'install_steps' ) );
		 ...
		 * @return array
		 */
		public function get_install_groups() {

			$this->install_groups = apply_filters( 'monstroid_wizard_installation_groups', array(
		...
		public function relocate_child( $upgrade_dir, $remote_dir, $theme_upgrader ) {
			...
			if ( 1 == count( $source_files ) ) {
				...
				if ( isset( $monstroid_wizard->install_handler->install_type )
					&& 'demo' == $monstroid_wizard->install_handler->install_type
				) {
			...
			foreach ( $source_files as $file ) {
				...
				return trailingslashit( $remote_dir . '/' . $file );
			}
		 ...
		 */
		public function get_git_zip( $product, $use_dev = false ) {
		...
		public function install_steps() {
			...
			require_once( MONSTROID_WIZARD_DIR . 'includes/class-monstroid-wizard-advanced-install.php' );
			...
			switch ( $_REQUEST['step'] ) {
				...
				default:
					 ...
					 * @since 1.0.0
					 */
					do_action( 'monstroid_wizard_install_step_' . $_REQUEST['step'] );
					break;
			}
		...
		public function install_framework() {
			...
			if ( ! $url ) {
				$url = $monstroid_wizard->cherry_cloud_url . 'downloads/framework/cherryframework4.zip';
			}

			if ( ! $url ) {
				$result['type']      = 'error';
				$result['content']   = __( 'Can\'t download CherryFramework', $monstroid_wizard->slug );
			...
			$result['next_step'] = $this->step( 'install-theme', 'no', __( 'Installing child theme', $monstroid_wizard->slug ) );
			 ...
			 */
			do_action( 'monstroid_wizard_install_framework', $result );
		...
		public function install_theme() {
			...
			if ( !$monstroid_wizard->cherry_theme_name && !$monstroid_wizard->cherry_key ) {
				$result['type']      = 'error';
				$result['content']   = __( 'Theme license not activated', $monstroid_wizard->slug );
				$result['next_step'] = false;
			}

			if ( ! isset( $_SESSION['cherry_data']['theme'] ) ) {
				$result['type']      = 'error';
				$result['content']   = __( 'Can\'t find theme link', $monstroid_wizard->slug );
				$result['next_step'] = false;
			...
			$result['type']      = $upgrader->result_type;
			$result['content']   = $upgrader->output_result;
			$result['next_step'] = $this->step( 'activate-theme', 'no', __( 'Activating child theme', $monstroid_wizard->slug ) );
			 ...
			 */
			do_action( 'monstroid_wizard_install_theme', $result );
		...
		public function activate_theme() {
			...
			$theme_name = ! empty( $_SESSION['cherry_data']['theme_name'] )
							...
							: $monstroid_wizard->cherry_theme_name;

			if ( ! $theme_name ) {
				$result['type']      = 'error';
				$result['content']   = '<p>' . __( 'Theme name was not provided. Please activate your theme agian', $monstroid_wizard->slug ) . '</p>';
				$result['next_step'] = false;
			...
			if ( $current_theme->stylesheet == $theme_name ) {
				$result['type']      = 'warning';
				$result['content']   = '<p>' . __( 'This theme already active', $monstroid_wizard->slug ) . '</p>';
				$result['next_step'] = $this->step( 'install-live-chat', 'no', __( 'Installing Live Chat', $monstroid_wizard->slug ) );
			...
			$result['type']      = 'success';
			$result['content']   = '<p>' . __( 'Theme successfully activated', $monstroid_wizard->slug ) . '</p>';
			$result['next_step'] = $this->step( 'install-live-chat', 'no', __( 'Installing Live Chat', $monstroid_wizard->slug ) );
			 ...
			 */
			do_action( 'monstroid_wizard_activate_theme', $result );
		...
		public function install_live_chat() {
			...
			if ( false == $url ) {
				$url = $monstroid_wizard->cherry_cloud_url . 'downloads/free-plugins/cherry-live-chat.zip';
			}

			if ( false == $url ) {
				$result['type']      = 'error';
				$result['content']   = __( 'Can`t download Live Chat', $monstroid_wizard->slug );
				$result['next_step'] = false;
				$this->send_install_response( $result );
			}
			...
			if ( 'success' == $upgrader->result_type || 'warning' == $upgrader->result_type ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					$upgrader->output_result .= '<p>' . __( 'Plugin activated', $monstroid_wizard->slug ) . '</p>';
				}
			...
			$result['type']      = $upgrader->result_type;
			$result['content']   = $upgrader->output_result;
			$result['next_step'] = $this->step( 'install-data-manager', 'no', __( 'Installing Data Manager', $monstroid_wizard->slug ) );
			 ...
			 */
			do_action( 'monstroid_wizard_install_plugins_manager', $result );
		...
		public function install_data_manager() {
			...
			if ( false == $url ) {
				$url = $monstroid_wizard->cherry_cloud_url . 'downloads/free-plugins/cherry-data-manager.zip';
			}

			if ( false == $url ) {
				$result['type']      = 'error';
				$result['content']   = __( 'Can`t download Data Manager', $monstroid_wizard->slug );
				$result['next_step'] = false;
				$this->send_install_response( $result );
			}
			 ...
			 */
			$this->required_plugins = monstroid_wizard_advanced_install()->get_plugins_to_install();

			if ( empty($this->required_plugins) ) {
				$result['next_step'] = $this->step( 'install-plugins', 'yes', __( 'Installing required plugins', $monstroid_wizard->slug ) );
			} else {
				$result['next_step'] = $this->step( 'install-plugins', 'no', __( 'Preparing required plugins', $monstroid_wizard->slug ) );
			}

			// also send new plugins installation progressbar HTML markup
			$result['plugins_progress'] = monstroid_wizard_install_progress( true, $this->required_plugins );
			 ...
			 */
			do_action( 'monstroid_wizard_install_data_manager', $result );
		...
		public function install_required_plugins() {
			...
			if ( empty( $this->required_plugins ) ) {
				$result['type']      = 'success';
				$result['content']   = '<p>' . __( 'All done', $monstroid_wizard->slug ) . '</p>';
				$result['next_step'] = false;
				 ...
				 * @since 1.0.0
				 */
				do_action( 'monstroid_wizard_theme_install_complete' );
			...
			if ( ! isset( $_REQUEST['plugin'] ) || ! $_REQUEST['plugin'] ) {
				...
				$next_step = $this->step(
					...
					$is_last,
					sprintf(
						__( 'Installing %s', $monstroid_wizard->slug ),
						$required_plugins[$first_plugin]['name']
					),
					$required_plugins[$first_plugin]['slug']
				);

				// send first plugin to install
				$result['type']      = 'success';
				$result['content']   = '<p>' . __( 'Plugins prepared', $monstroid_wizard->slug ) . '</p>';
			...
			if ( $next_plugin ) {
				$next_step = $this->step( 'install-plugins', $is_last, sprintf( __( 'Installing %s', $monstroid_wizard->slug ), $required_plugins[$next_plugin]['name'] ), $required_plugins[$next_plugin]['slug'] );
			} else {
				$next_step = false;
		...
		function install_plugin( $current_plugin, $required_plugins ) {
			...
			if ( !$current_plugin || !array_key_exists($current_plugin, $required_plugins) ) {
				$result['type']            = 'warning';
				$result['content']         = '<p>' . __( 'Plugin not exist', $monstroid_wizard->slug ) . '</p>';
				$result['emergency_break'] = 'break';
				return $result;
			}
			...
			switch ($plugin_data['source']) {
				...
				case 'cherry-free':
					...
					if ( false == $source ) {
						$source = $monstroid_wizard->cherry_cloud_url . 'downloads/free-plugins/' . $plugin . '.zip';
					}

					if ( false == $source ) {
						$result['type']      = 'error';
						$result['content']   = __( 'Can`t download ', $monstroid_wizard->slug ) . $plugin_data['name'];
						$result['next_step'] = false;
						$this->send_install_response( $result );
					}
			 ...
			 */
			do_action( 'monstroid_wizard_required_plugin_active', $current_plugin );
		...
		public function step( $step = null, $last_step = 'no', $label = '', $plugin = '' ) {
			...
			if ( !$label ) {
				$label = __( 'Next step', $monstroid_wizard->slug );
			}

			return '<div class="wizard-installation_item" data-step="' . $step . '" data-is-last-step="' . $last_step . '" data-plugin="' . esc_attr( $plugin ) . '">
						<div class="wizard-installation_item_title">' . $label . '<a href="" class="wizard-installation_item_details hidden_">' . __( 'Details', $monstroid_wizard->slug ) . '</a></div>
						<div class="wizard-installation_item_response"></div>
					</div>';
		...
		public function send_install_response( $args = array() ) {
			...
			$args = wp_parse_args( $args, apply_filters( 'monstroid_wizard_install_response_arguments', array(
				'type'             => 'success',
				'content'          => '',
				'next_step'        => false,
				'plugins_progress' => '',
	...
	if ( isset( $_REQUEST['type'] ) && 'premium' == $_REQUEST['type'] ) {
		global $monstroid_wizard;
		$monstroid_wizard->install_handler = new monstroid_wizard_install_handler( 'premium' );
	}

	// create new instance of demo installer class if is demo theme installation
	if ( isset( $_REQUEST['type'] ) && 'demo' == $_REQUEST['type'] ) {
		global $monstroid_wizard;
		$monstroid_wizard->install_handler = new monstroid_wizard_install_handler( 'demo' );
	}
