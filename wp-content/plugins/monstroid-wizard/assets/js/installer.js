...
jQuery(document).ready( function( $ ) {
	...
	var step_event = jQuery.Event('monstroid_wizard_step_response');

	function monstroid_wizard_ajax_request() {
		...
		$.ajax({
			...
			dataType: "json",
			data: {
				action: 'monstroid_wizard_install_step',
				step: _step,
				type: _type,
				plugin: _plugin
			},
			...
			if ( _is_last_step == 'yes' ) {
				...
				_item.after('<div class="submit-wrap_">' + monstroid_wizard_install_data.redirect_message + '</div>');

				window.location = monstroid_wizard_steps.content_install;
			}
		})

	}

	function monstroid_wizard_progress_item_length() {
	...
	function monstroid_wizard_update_plugins_progress( step, response ) {
	...
	if ( 'ok' == monstroid_wizard_install_data.dir_permissions && 'ok' == monstroid_wizard_install_data.server_settings ) {
		monstroid_wizard_ajax_request();
	} else if ( 'error' == monstroid_wizard_install_data.dir_permissions && 'error' == monstroid_wizard_install_data.server_settings ) {
	...
	monstroid_wizard_progress_item_length();

	// installation progress bar
	var plugins_progress = 0;
	$(document).on( 'monstroid_wizard_step_response', function( event, step, plugin, response ) {
		event.preventDefault();

		if ( plugins_progress == 0 ) {
			plugins_progress = monstroid_wizard_update_plugins_progress( step, response );
			if ( plugins_progress != 0 ) {
