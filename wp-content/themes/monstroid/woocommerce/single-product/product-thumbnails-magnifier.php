...
if ( $attachment_ids ) {
	...
	<div class="thumbnails <?php echo $enable_slider ? 'slider' : 'noslider' ?>">
		...
		foreach ( $attachment_ids as $attachment_id ) {
			...
			echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '<li class="%s"><a href="%s" class="%s" title="%s" data-small="%s"><div class="border-wrap">%s</div></a></li>', $image_class, $magnifier_url, $image_class, $image_title, $thumbnail_url, $image ), $attachment_id, $post->ID, $image_class );

			$loop++;
		}

		/*if (count($attachment_ids) < 5) {
			cherry_woocommerce_get_missed_thumb(count($attachment_ids));
		}*/
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		jQuery('#open-gallery-popup').magnificPopup({
			items: [
				<?php
					$loop = 0;
					foreach ( $attachment_ids as $attachment_id ) {
						list( $magnifier_url, $magnifier_width, $magnifier_height ) = wp_get_attachment_image_src( $attachment_id, "full" );
						echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', sprintf( '{src: "%s"},', $magnifier_url ), $attachment_id, $post->ID, $image_class );
						$loop++;
					}
				?>
			],
